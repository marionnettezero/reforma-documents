...
### 4. S3 の temp 領域のライフサイクル設定

フォームインポート（ZIP/JSON）、テーマインポート、フォーム項目・選択肢のCSVインポートなどでは、複数台Web構成向けに一時ファイルを S3 の **temp** プレフィックス配下に保存しています。ジョブ完了時（成功・失敗とも）にアプリ側でオブジェクトを削除しますが、ジョブが一度も実行されなかった場合（ワーカー停止・障害など）や、実行中のクラッシュでは削除されず残ることがあります。

そのため、**取りこぼしを自動で片付ける**ため、S3 のライフサイクルルールで `temp/` プレフィックスのオブジェクトを「一定日数経過後に削除」する設定をしておくことを推奨します。

**対象プレフィックス（用途）**

| プレフィックス | 用途 |
|----------------|------|
| `temp/form-import/` | フォーム ZIP/JSON インポートの一時ファイル |
| `temp/theme-import/` | テーマ CSV インポートの一時ファイル |
| `temp/forms-fields-import/` | フォーム項目 CSV（全項目／選択肢）の一時ファイル |
| `temp/field-options-import/` | 特定フィールドの選択肢 CSV の一時ファイル |

**手順（AWS マネジメントコンソール）**

1. [S3](https://console.aws.amazon.com/s3/) で、ReForma が利用するバケットを開く
2. **オブジェクトの管理** タブ（または **管理**）→ **ライフサイクルルール** → **ライフサイクルルールを作成**
3. ルール名を入力（例: `Delete temp files after 1 day`）
4. **オブジェクトのフィルター** で「プレフィックスを制限」に `temp/` を指定
5. **ライフサイクルルールのアクション** で「有効期限」を選び、日数を指定（例: **1** 日）
6. 作成して有効化

**ルール例（コンソールで設定する場合のイメージ）**

- **プレフィックス**: `temp/`
- **アクション**: 現在のバージョンのオブジェクトの有効期限
- **日数**: `1`（24時間経過後に削除）

**注意**

- 有効期限の日数は運用方針に合わせて変更してください（1日で十分な場合が多く、より短い時間で消したい場合は、日次実行のバッチで `temp/` 以下を削除する方法も検討できます）
- ライフサイクルによる削除は**追加コストなし**で利用できます
- `temp/` 以外のプレフィックス（`pdf-templates/`、`archives/` など）には適用しないでください
