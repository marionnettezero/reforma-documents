...
1. **ウィザード形式**（新規作成時）
   ...
   #### 概要
   フォーム作成を段階的なウィザード形式に変更し、各ステップで必要な設定のみを表示することで、ユーザーの迷いを減らし、初心者にも分かりやすい操作フローを提供する。
   ...
   **ステップ1: 基本情報**
   ...
   - フォームタイプの選択（STEP式/単一式）
     - STEP式: 複数のステップに分けて回答を収集
     - 単一式: 1画面で全ての項目を表示
   - バリデーション: フォーム名は必須、公開期間の整合性チェック
   
   **ステップ2: 構造設計**
   - STEP式の場合:
     - STEPの追加・編集（STEP名、説明、順序）
     - GROUPの追加・編集（GROUP名、説明、所属STEP）
     - ドラッグ&ドロップでSTEP/GROUPの順序変更
   - 単一式の場合:
     - GROUPの追加・編集（GROUP名、説明）
     - ドラッグ&ドロップでGROUPの順序変更
   - カードベースの視覚的UIで階層構造を表示
   - 最小構成: STEP式は最低1つのSTEP、各STEP/GROUPには最低1つのGROUP/項目が必要（ステップ3で追加）
   
   **ステップ3: 項目追加**
   - 項目タイプの選択（text, email, number, select, radio, checkbox, textarea, date, file, computed など）
   - 基本設定:
     - フィールドキー（自動生成または手動入力）
     - ラベル（多言語対応）
     - 必須設定（必須/任意）
     - プレースホルダー（任意）
   - 簡易プレビュー: 選択した項目タイプの見た目を即座に確認
   - 複数項目の一括追加に対応
   - バリデーション: フィールドキーの重複チェック、ラベルの必須チェック
   ...
   **ステップ4: 詳細設定**（必要に応じて）
   - 選択肢の設定（select/radio/checkbox用）:
     ...
     - デフォルト値の設定
   - 条件分岐ルール:
     - 表示条件: 特定の項目の値に応じて表示/非表示を制御
     - 必須条件: 特定の条件を満たした場合のみ必須にする
     - ステップ遷移条件: STEP式の場合、条件に応じて次のステップを決定
   - 計算ルール（computed用）:
     - 計算式エディタ
     - 参照可能な項目の一覧表示
     - 計算結果のプレビュー
   - 高度な設定:
     - PDF設定（PDFエクスポート時の表示設定）
     - バリデーションルール（文字数制限、数値範囲など）
     - ヘルプテキスト
   
   **ステップ5: 確認・公開**
   - フォーム全体のプレビュー:
     - 公開フォーム画面（U-01）と同じ見た目で表示
     - 条件分岐の動作確認
     - レスポンシブデザインの確認
   - 最終確認:
     - 基本情報の確認
     - 構造の確認（STEP/GROUP/項目の一覧）
     - 設定の確認（必須項目、条件分岐、計算ルールなど）
   - 公開設定:
     - 公開状態（下書き/公開）
     - 通知設定（ユーザー通知/管理者通知）
     - テーマ設定（テーマID、テーマトークン）
   ...
   - **進捗表示**: ステップインジケーターで現在位置と全体の進捗を表示（例: ステップ2/5）
   - **ナビゲーション**:
     - 「次へ」ボタン: 現在のステップのバリデーション通過後に次のステップへ
     - 「戻る」ボタン: 前のステップに戻る（入力内容は保持）
     - 「スキップ」ボタン: 任意のステップ（ステップ4など）をスキップ可能
   - **データ保持**: 各ステップの入力内容は一時保存され、ブラウザを閉じても復元可能（IndexedDB使用）
   - **バリデーション**: 各ステップでリアルタイムバリデーションを実行し、エラーがある場合は次のステップに進めない
   - **保存機能**: 任意のステップで「下書き保存」が可能。保存後は編集画面（統合UI）に遷移
   
   #### API連携
   
   - **ステップ1完了時**: `POST /v1/forms` でフォームを作成（下書き状態）
   - **ステップ2完了時**: `PUT /v1/forms/{id}` でフォーム構造を更新
   - **ステップ3完了時**: `POST /v1/forms/{id}/items` で項目を追加、または `PUT /v1/forms/{id}/fields` で一括更新
   - **ステップ4完了時**: `PUT /v1/forms/{id}/fields` で項目の詳細設定を更新
   - **ステップ5完了時**: `PUT /v1/forms/{id}` で公開設定を更新し、必要に応じて通知設定も更新
   
   #### 実装コンポーネント
   
   - **FormWizard**: ウィザード形式のコンテナコンポーネント
     - ステップ管理（現在のステップ、進捗表示）
     - ナビゲーション（次へ/戻る/スキップ）
     - データ永続化（IndexedDB）
   - **WizardStep**: 各ステップのラッパーコンポーネント
     - ステップ固有のバリデーション
     - ステップ間のデータ受け渡し
   - **FormStructureEditor**: STEP/GROUPの構造編集コンポーネント（ステップ2で使用）
     - カードベースの視覚的UI
     - ドラッグ&ドロップ対応
   - **FieldTypeSelector**: 項目タイプ選択コンポーネント（ステップ3で使用）
     - 項目タイプの一覧表示
     - 簡易プレビュー
   - **FieldDetailEditor**: 項目詳細設定エディタ（ステップ4で使用）
     - 選択肢設定
     - 条件分岐ビルダー
     - 計算式エディタ
   - **FormPreview**: フォームプレビューコンポーネント（ステップ5で使用）
     - 公開フォーム画面（U-01）と同じレンダリング
     - 条件分岐の動作確認
   ...
   - ✅ 各ステップで集中できる（情報過多を回避）
   - ✅ 進捗状況が明確（ステップインジケーター）
   - ✅ 初心者にも分かりやすい（ガイド付きの操作フロー）
   - ✅ 必須項目と任意項目が明確
   - ✅ バリデーションエラーを段階的に解決できる
   
   #### デメリット・注意点
   
   - ❌ 既存のフォーム編集時は少し手間（編集時は統合UIを使用）
   - ❌ 画面遷移が増える（ただし、各ステップで集中できるため実質的な負担は軽減）
   - ⚠️ 大量の項目がある場合、ステップ3・4の処理時間が長くなる可能性（仮想スクロールやページネーションで対応）
   - ⚠️ ブラウザを閉じた場合の復元処理が必要（IndexedDBを使用）
   
   #### 実装優先度
   
   - **フェーズ2（中期）**: 統合UIの改善と並行して実装
   - 新規作成時のみウィザード形式を使用し、既存フォームの編集時は統合UI（カードベース）を使用するハイブリッド方式を推奨

2. **リアルタイムプレビュー**
   - 設定変更時の即座のプレビュー反映
   
   #### 概要
   フォーム編集時に、設定変更をリアルタイムでプレビューできる機能。右サイドパネルにタブで統合し、「フィールド詳細」と「プレビュー」を切り替え可能にする。
   
   #### 表示場所の提案（推奨案: 右サイドパネルとタブ切り替え）
   ...
   **レイアウト構造**:
   ...
   │             │                      │              │
   │ ・基本情報    │ ・フォーム構造編集      │  ┌──────────┐│
   │ ・通知設定    │ ・FormStructureEditor │  │ プレビュー ││
   │ ・テーマ設定  │                      │  │ コンテンツ ││
   │             │                      │  │          ││
   │             │                      │  └──────────┘│
   └─────────────┴──────────────────────┴──────────────┘
   ...
   **メリット**:
   - ✅ 既存のレイアウト構造を維持（3カラムレイアウト）
   - ✅ 画面領域を追加で消費しない
   - ✅ フィールド編集とプレビューを同じ場所で切り替え可能
   - ✅ 実装が容易（既存の右サイドパネルを拡張）
   - ✅ 段階的実装が可能（将来的に分割表示などに拡張可能）
   ...
   **FormPreviewPanelコンポーネント**:
   - フォーム構造、フィールド、翻訳、テーマの変更をリアルタイムで反映
   - 公開フォーム画面（U-01）と同じレンダリングエンジンを使用
   - 条件分岐の動作確認が可能
   - レスポンシブデザインの確認が可能
   
   **動作仕様**:
   - フォーム構造の変更（STEP/GROUP/項目の追加・削除・順序変更）を即座に反映
   - フィールド設定の変更（ラベル、必須、選択肢、条件分岐など）を即座に反映
   - 翻訳の変更を即座に反映
   - テーマ設定の変更を即座に反映
   - 条件分岐の動作を視覚的に確認可能
   
   **自動切り替え（オプション）**:
   - フィールドを選択した時は自動的に「フィールド詳細」タブに切り替え
   - フィールド選択を解除した時は自動的に「プレビュー」タブに切り替え
   
   #### その他の表示場所案
   
   **案2: メインエリアと分割表示（上下分割）**
   - メインエリアを上下に分割
   - 上部: フォーム構造編集、下部: リアルタイムプレビュー
   - メリット: 編集とプレビューを同時に確認可能
   - デメリット: 各エリアの高さが制限される、実装がやや複雑
   
   **案3: メインエリアと分割表示（左右分割）**
   - メインエリアを左右に分割
   - 左側: フォーム構造編集、右側: リアルタイムプレビュー
   - メリット: 編集とプレビューを同時に確認可能、大画面で効果的
   - デメリット: 各エリアの幅が制限される、小画面では使いにくい
   
   **案4: フローティングプレビューパネル**
   - プレビューをフローティングパネルとして表示
   - 画面右下に固定位置で表示、ドラッグで移動可能
   - メリット: 既存レイアウトに影響しない
   - デメリット: 実装が複雑、他のUI要素と重なる可能性
   
   **案5: 別タブ/ウィンドウ**
   - プレビューを別タブまたは別ウィンドウで開く
   - メリット: 既存レイアウトに全く影響しない、大画面で効果的
   - デメリット: タブ/ウィンドウの切り替えが必要、リアルタイム性がやや低下
   
   #### 実装優先度
   
   - **フェーズ3（長期）**: 統合UIの改善と並行して実装
   - **推奨実装**: 案1（右サイドパネルとタブ切り替え）を最優先で実装
   - **将来の拡張**: 案2や案3（分割表示）への移行も検討可能
   
   #### 参考資料
   
   - `REALTIME_PREVIEW_PLACEMENT_PROPOSAL.md`: リアルタイムプレビューの表示場所に関する詳細な提案
   - 公開フォーム画面（U-01）のレンダリングエンジンを再利用
