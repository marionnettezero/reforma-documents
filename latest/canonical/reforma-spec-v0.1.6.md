...
| --- | --- | --- | --- |
| api | v0.1.4 | latest/classified/api/reforma-api-spec-v0.1.4.json | latest/classified/api/reforma-api-spec-v0.1.4.md |
| backend | v1.1.0 | latest/classified/backend/reforma-backend-spec-v1.1.0.json | latest/classified/backend/reforma-backend-spec-v1.1.0.md |
...
| db | v0.1.1 | latest/classified/db/reforma-db-spec-v0.1.1.json | latest/classified/db/reforma-db-spec-v0.1.1.md |
| frontend | v1.1.0 | latest/classified/frontend/reforma-frontend-spec-v1.1.0.json | latest/classified/frontend/reforma-frontend-spec-v1.1.0.md |
...
### PUT /v1/forms/{id}
- **概要**: フォーム更新
- **説明**: フォーム情報を更新する。期間設定、添付ファイル設定、通知設定などを更新可能。
- **パラメータ**:
...
- **リクエストボディ**:
  - コンテンツタイプ: application/json
    - スキーマ型: object
    - フィールド:
      - `status` (string, enum: draft/published/closed): フォームステータス
      - `is_public` (boolean): 公開フラグ
      - `public_period_start` (string, date-time, nullable): 公開期間開始日時
      - `public_period_end` (string, date-time, nullable): 公開期間終了日時
      - `answer_period_start` (string, date-time, nullable): 回答期間開始日時
      - `answer_period_end` (string, date-time, nullable): 回答期間終了日時
      - `attachment_enabled` (boolean): 添付ファイル機能が有効か
      - `attachment_type` (string, nullable, enum: pdf_template/uploaded_files): 添付ファイルタイプ
      - `notification_user_enabled` (boolean): ユーザー通知が有効か
      - `notification_user_email_template` (string, nullable): ユーザー通知メールテンプレート
      - `notification_user_email_subject` (string, nullable): ユーザー通知メール件名
      - `notification_user_email_from` (string, nullable, email): ユーザー通知メール送信元アドレス
      - `notification_user_email_reply_to` (string, nullable, email): ユーザー通知メール返信先アドレス
      - `notification_user_email_cc` (array[string], nullable): ユーザー通知メールCC（配列）
      - `notification_user_email_bcc` (array[string], nullable): ユーザー通知メールBCC（配列）
      - `notification_admin_enabled` (boolean): 管理者通知が有効か
      - `notification_admin_user_ids` (array[integer], nullable): 管理者通知送信先ユーザーID一覧
      - `notification_admin_email_template` (string, nullable): 管理者通知メールテンプレート
      - `notification_admin_email_subject` (string, nullable): 管理者通知メール件名
      - `notification_admin_email_from` (string, nullable, email): 管理者通知メール送信元アドレス
      - `notification_admin_email_reply_to` (string, nullable, email): 管理者通知メール返信先アドレス
      - `translations` (array): 翻訳データ
- **レスポンス**:
  - 200: OK
    - コンテンツタイプ: application/json
      - スキーマ型: object
...
- **概要**: 公開 submit
- **説明**: フォーム送信。条件分岐がある場合、成功時は `data.condition_state` を返す。STEP 遷移が拒否された場合は 422 で `errors.step` を返す。公開期間・回答期間のチェックが行われ、期間外の場合は403エラーを返す。送信後、通知ジョブがキューに投入される。
- **パラメータ**:
  - `form_key` (パス, 必須, 型: string): フォーム公開キー
- **リクエストボディ**:
  - コンテンツタイプ: application/json
    - スキーマ型: object
- **レスポンス**:
  ...
  - 403: 403 Forbidden（公開期間外または回答期間外）
    - コンテンツタイプ: application/json
      - スキーマ型: #/components/schemas/EnvelopeError
  - 422: 422 Validation Error
    - コンテンツタイプ: application/json
...
- **概要**: 公開フォーム表示取得
- **説明**: Respondent 用フォーム表示（/f/{form_key}）。条件分岐がある場合は `data.condition_state` を返す。公開期間のチェックが行われ、期間外の場合は403エラーを返す。
- **パラメータ**:
  - `form_key` (パス, 必須, 型: string): フォーム公開キー
- **レスポンス**:
  - 200: OK
...
| field_key | varchar(255) | NO | - | フィールド識別子 |
| value | text | YES | NULL | 値 |
| label | text | YES | NULL | ラベル（スナップショット） |
| created_at | timestamp | YES | NULL | 作成日時 |
| updated_at | timestamp | YES | NULL | 更新日時 |

**インデックス**:
- `idx_submission_values_submission_id`: (submission_id)
- `uk_submission_values_submission_id_field_key`: (submission_id, field_key) UNIQUE
